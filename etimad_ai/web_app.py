...
if uploaded_file:
    ...
    if st.button(
        ...
        try:
            ...
            st.header("🎯 لوحة القرار")

            st.info(
                "هذه اللوحة تلخص أهم بيانات المنافسة "
                "المستخرجة من تحليل Etimad AI."
            )

            # نحاول استخراج درجة الملاءمة من نتيجة Gemini
            import re

            score = "غير مستخرج"

            score_match = re.search(
                r"درجة ملاءمة الشركة.*?(\d+)\s*(?:من|/)\s*100",
                result,
                re.IGNORECASE
            )
            ...
            deadline_match = re.search(
                r"(?:الموعد النهائي|تاريخ الموعد النهائي).*?"
                r"(30 سبتمبر 2026|30 September 2026)",
                result,
                re.IGNORECASE
            )
            ...
            if "مناسب مبدئيًا" in result:

                st.success(
                    "🟢 مناسب مبدئيًا"
                )

            elif "يحتاج تحقق إضافي" in result:

                st.warning(
                    "🟠 يحتاج تحقق إضافي"
                )

            elif "غير مناسب مبدئيًا" in result:
                ...
                st.info(
                    "راجع نتيجة التحليل لاتخاذ القرار."
                )
